...
import firebase from 'firebase/compat/app'; // Asegura que se importa desde compat
import 'firebase/compat/firestore'; // Asegura compatibilidad con Firestore

// Interfaces para usuario y mensajes
...
export class ChatService {
  currentUser: User | null = null; // Asegura que currentUser puede ser null

  constructor(private afAuth: AngularFireAuth, private afs: AngularFirestore) {
    this.afAuth.authState.subscribe((user) => {
      if (user) {
        this.currentUser = {
          uid: user.uid ?? '', // Si es null o undefined, asigna una cadena vacía
          email: user.email ?? '',
        };
      } else {
